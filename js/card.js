...
(function () {
  ...
  function createCard(offer, closeHandler) {
    ...
    cardAd.querySelector('.popup__avatar').src = offer.author.avatar || NO_VALUE;

    cardAd.querySelector('.popup__title').textContent = offer.offer.title || NO_VALUE;

    cardAd.querySelector('.popup__text--address').textContent = offer.offer.address || NO_VALUE;

    cardAd.querySelector('.popup__text--price').textContent = offer.offer.price + '₽/ночь' || NO_VALUE;

    cardAd.querySelector('.popup__type').textContent = createTypeCard(offer) || NO_VALUE;

    cardAd.querySelector('.popup__text--capacity').textContent = offer.offer.rooms + ' комнаты для ' + offer.offer.guests + ' гостей' || NO_VALUE;

    cardAd.querySelector('.popup__text--time').textContent = 'Заезд после ' + offer.offer.checkin + ', выезд до ' + offer.offer.checkout || NO_VALUE;

    cardAd.querySelector('.popup__description').textContent = offer.offer.description || NO_VALUE;
    ...
    childElements.forEach(function (element) {
      if (element.textContent === NO_VALUE) {
